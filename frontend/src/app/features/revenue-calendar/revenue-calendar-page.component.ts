...
export class RevenueCalendarPageComponent {
  ...
  onExportPdf(): void {
    ...
    const reportId = `${range.startDate}_to_${range.endDate}`;

    this.reportsApiService.obtenerRevenueCalendarPdf(reportId, range).subscribe({
      next: (blob) => {
      ...
      error: () => {
        this.errorMessage.set('No fue posible abrir el PDF de Revenue Calendar.');
        this.printing.set(false);
      }
    });
  }
